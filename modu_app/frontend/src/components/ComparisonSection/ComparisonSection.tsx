...
interface DimensionComparison {
    dimension: string;
    winner: "A" | "B" | "tie";
    a_status: "compliant" | "non_compliant" | "missing" | "partial" | "clarification_needed" | "unclear";
    b_status: "compliant" | "non_compliant" | "missing" | "partial" | "clarification_needed" | "unclear";
    rationale: string;
}
...
const statusClass: Record<string, string> = {
    ...
    clarification_needed: "status-clarification",
...
function WinnerBadge({ winner }: { winner: "A" | "B" | "tie" }) {
    const cls = winner === "tie" ? "winner-badge tie" : "winner-badge win";
    const text = winner === "tie" ? "TIE" : `PKG ${winner} WINS`;
...
export function ExpandedComparison({ record }: { record: ComparisonRecord }) {
        ...
        <div className="cmp-expanded">
            <div className="cmp-scores-row">
                <div className="cmp-score-block">
                    <div className="cmp-pkg-label">PKG A</div>
                ...
                <div className="cmp-score-block">
                    <div className="cmp-pkg-label">PKG B</div>
                ...
                <div className="cmp-section-block">
                    ...
                    <ul className="cmp-list">
                        {r.a_exclusive_strengths?.filter(s => !s.toLowerCase().startsWith("none")).length > 0
                            ? r.a_exclusive_strengths.map((s, i) => <li key={i}>{s}</li>)
                            : <li>No exclusive strengths found for PKG A</li>
                        }
                    </ul>
                </div>
                <div className="cmp-section-block">
                    ...
                    <ul className="cmp-list">
                        {r.b_exclusive_strengths?.filter(s => !s.toLowerCase().startsWith("none")).length > 0
                            ? r.b_exclusive_strengths.map((s, i) => <li key={i}>{s}</li>)
                            : <li>No exclusive strengths found for PKG B</li>
                        }
                    </ul>
                </div>
            </div>
                ...
                <div className="cmp-section-block">
                    ...
                    <ul className="cmp-list failure-list">
                        {r.a_critical_failures?.filter(s => !s.toLowerCase().startsWith("none")).length > 0
                            ? r.a_critical_failures.map((s, i) => <li key={i}>{s}</li>)
                ...
                <div className="cmp-section-block">
                    ...
                    <ul className="cmp-list failure-list">
                        {r.b_critical_failures?.filter(s => !s.toLowerCase().startsWith("none")).length > 0
                            ? r.b_critical_failures.map((s, i) => <li key={i}>{s}</li>)
            ...
            <div className="cmp-section-block">
                ...
                <ul className="cmp-list shared-list">
                    {r.shared_deficiencies?.filter(s => !s.toLowerCase().startsWith("none")).length > 0
                        ? r.shared_deficiencies.map((s, i) => <li key={i}>{s}</li>)
