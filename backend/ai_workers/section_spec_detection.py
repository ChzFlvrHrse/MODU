...
async def section_spec_detection_ai(rasterized_pages: list[dict[int, bytes]], section_number: str, section_title: str) -> list[int]:
    content = [
        {
            "type": "text",
            "text": "Analyze the following pages of a spec sheet and detect ALL pages that contain the section number and/or section title."
        },
        # Flatten: for each page, add PAGE_INDEX text + image_url
        *[
            item
            ...
            for item in (
                {
                    "type": "text",
                    "text": f"PAGE_INDEX: {page['page_index']}",
                },
                {
                    "type": "image_url",
                    "image_url": {
                        "url": "data:image/png;base64,"
                        + base64.b64encode(page["bytes"]).decode("utf-8")
                    },
                },
            )
        ],
        {"type": "text", "text": f"Section number: {section_number}"},
        {"type": "text", "text": f"Section title: {section_title}"},
    ]
